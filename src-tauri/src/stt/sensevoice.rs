...
impl STTProvider for SenseVoiceProvider {
    fn transcribe_blocking(
        ...
        std::fs::write(&input_path, &wav)?;

        let result = run_sidecar_sync(self.app_handle.clone(), &self.model_path, &input_path);
...
fn run_sidecar_sync(
    ...
    let output = tokio::runtime::Handle::current().block_on(async move {
        app_handle
            .shell()
