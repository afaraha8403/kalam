...
## [Unreleased]
- **Change:** `docs/download.html` visual refresh: Fraunces display headline, layered mesh/noise hero, status row with pulse → success state, OS detection chip, icon-led “While you wait” panel, three permission cards, richer disclosures and file-type badges on asset rows; respects `prefers-reduced-motion`.
- **Change:** `package.json`: add `docs:dev` (static server for `docs/` on port 4321); remove redundant `tauri:dev` alias (use `npm run dev`).
- **Change:** Update notifications (background check + tray “Check for Updates”) now say to use **Settings → About** to download/install, matching in-app updater UX.
- **Feature:** Marketing site `download.html`: auto-starts the correct GitHub release asset for the visitor’s OS, OS-specific security prompt guides (SmartScreen / Gatekeeper / Linux), permissions heads-up, and an “All downloads” list (latest release assets) open by default.
- **Change:** Landing download CTA points to `download.html` instead of linking straight to a single GitHub URL; label still reflects detected OS when the releases API succeeds.
- **Feature:** Settings → About → Updates: **Update now** (download, install, restart immediately) and **Update on next start** (install without restart; message confirms it applies next launch).
- **Change:** Tauri command `download_and_install_update` takes a `restart` flag; restart is skipped when the user chooses next-start install.
- **Fix:** Hotkey UI shows the correct OS/meta key label: Win (Windows), Cmd (macOS), Super (Linux); stored spellings (Win/Super/Cmd/Command/Meta) normalize for display. Onboarding uses `macos` from `get_platform` (Rust `std::env::consts::OS`) instead of `darwin`.
- **Fix:** Overlay collapsed hint and onboarding toggle hotkey use the same platform-specific meta display as the rest of the UI; `settings_updated` refreshes `sidebarDictationStore` so the status bar hotkey text updates after saves.
