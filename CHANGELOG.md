...
- **Change:** Cloud STT: tighter per-clip timeout floor (`12 + min(audio, 20)/2` seconds, capped at 22s) so long recordings don't balloon the deadline; local STT floors unchanged.
- **Feature:** Overlay processing UX: "Transcribing" after ~3s, "Slow response..." from half of expected time (min 8s) with elapsed counter; immediate "Retry N/3" on retry start; cancel button (hover or after 5s); success checkmark delayed up to 1.5s after a retry so the retry badge stays visible.
- **Change:** Overlay pill processing layout: cancel button is now in-flow (flex child) instead of absolutely positioned, preventing overlap with the elapsed timer; smaller font sizes and tighter spacing.
